...
def init_db() -> None:
    ...
    # Инициализация calendar_holidays.json на volume (persistent)
    holidays_volume_path = os.path.join(os.path.dirname(abs_path), "calendar_holidays.json")
    if not os.path.exists(holidays_volume_path):
        holidays_src = os.path.join(os.path.dirname(__file__), "calendar_holidays.json")
        if os.path.exists(holidays_src):
            import shutil
            shutil.copy2(holidays_src, holidays_volume_path)
            print(f"[INIT] calendar_holidays.json скопирован на volume: {holidays_volume_path}")
        else:
            print(f"[INIT] ВНИМАНИЕ: calendar_holidays.json не найден ни на volume, ни в app/")
    else:
        print(f"[INIT] calendar_holidays.json уже на volume: {holidays_volume_path}")

    Base.metadata.create_all(bind=engine)

    # Миграция: добавляем новые колонки в существующие таблицы SQLite
    with engine.connect() as conn:
        try:
            conn.execute(text("ALTER TABLE employees ADD COLUMN department VARCHAR(255) DEFAULT ''"))
            conn.commit()
            print("[INIT] Миграция: добавлена колонка department")
        except Exception:
            pass  # колонка уже существует
        try:
            conn.execute(text("ALTER TABLE employees ADD COLUMN carry_over_days INTEGER DEFAULT 0"))
            conn.commit()
            print("[INIT] Миграция: добавлена колонка carry_over_days")
        except Exception:
            pass
        try:
            conn.execute(text("ALTER TABLE employees ADD COLUMN color VARCHAR(7) DEFAULT '#3b82f6'"))
            conn.commit()
            print("[INIT] Миграция: добавлена колонка color")
        except Exception:
            pass
